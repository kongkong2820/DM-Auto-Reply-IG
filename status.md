...
실제 테스트에서 댓글만 작성한 사용자는 팔로우 조회가 code 230으로 실패하고, 사용자가 먼저 DM 상호작용을 하면 조회와 발송이 성공함을 확인했다. 이를 반영해 댓글에는 최초 안내와 빠른 답장 버튼을 보내고 버튼 선택 후 팔로우를 확인하는 흐름을 구현 중이다. `is_user_follow_business === true`일 때만 최종 메시지를 보내며 나머지는 재확인 안내를 보낸다.
...
1. 빠른 답장 코드를 배포한다. 운영 D1 migration은 적용 완료했다.
2. Meta Webhook에서 `messages` 필드를 구독한다.
