...
Cloudflare Workers Builds의 GitHub 연결은 Dashboard의 일회성 GitHub App 승인이 필요해 진행 중이다.
...
| 기존 URL `/` | 운영 URL GET | 200 | 200, `autoReply: true` | 배포 전 기준 통과 |
| 기존 URL `/health` | 운영 URL GET | 설정 정상 | 200, 자동응답·키워드·메시지 설정됨 | 배포 전 기준 통과 |
| 기존 URL `/privacy` | 운영 URL GET | HTML 200 | HTML 200 | 배포 전 기준 통과 |
| Workers Builds | Dashboard `Settings → Builds` | GitHub `main` 연결 | GitHub App 승인 대기 | 진행 중 |
| Webhook·실제 DM | 배포 후 실제 환경 | 정상 수신·발송 | 미실행 | 대기 |
...
브라우저에서 Cloudflare GitHub App을 승인하고 저장소 `kongkong2820/DM-Auto-Reply-IG`, 브랜치 `main`, 루트 `/`, 배포 명령 `npx wrangler deploy`를 기존 Worker에 연결해야 한다. 연결 후 Git 빌드 배포와 기존 URL, Webhook, 실제 댓글 → DM을 검증해야 TASK-02를 완료할 수 있다.
